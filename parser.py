...
-------------------------------------
This module provides functionality to parse Windows Event Logs from two sources:

  1. OFFLINE  - In the `data/evtx/` directory, this module reads .evtx files using python-evtx.
                It works in server/analytics environments (including Linux/macOS),
                as python-evtx parses the file at the bytecode level, without requiring the Windows API.

  2. LIVE     - In the Windows system, this module monitors the Security and Sysmon channels in real-time using `pywin32` (win32evtlog).
...
class UserBehaviorEvent(BaseModel):
    ...
    feature_extractor.py and risk_engine.py only operate on this schema.
    """
    ...
    # All the normalized fields below are optional, as not all events contain them.
    ...
    target_object: Optional[str] = None   # registry/persistence events
    target_filename: Optional[str] = None  # filesystem events

    # Remaining unnormalized fields (for audit/debug purposes)
    raw: Dict[str, Any] = Field(default_factory=dict)


# A function that maps raw event_data to UserBehaviorEvent.

# Difference between various event sources (Security, Sysmon) is normalized here.
# The mapping is based on the event ID and the known structure of event_data for that ID.
...
class WindowsEventLogParser:
    """
    Reads EVTX files (offline) or the live Windows Event Log (live, Windows only) 
    and generates a normalized stream of `UserBehaviorEvent`s.
    """
    ...
    def parse_evtx_file(self, file_path: Path) -> Iterator[UserBehaviorEvent]:
        """
        Reads a single .evtx file line-by-line (lazily). The `python-evtx`
        library is required: pip install python-evtx
        """
        try:
            from evtx.evtx import Evtx  # python-evtx package
        except ImportError as exc:
            raise ImportError(
                "python-evtx package not found. Install it: pip install python-evtx"
            ) from exc
        ...
        try:
            with Evtx(str(file_path)) as log:
                for record in log.records():
                    ...
                    except Exception as exc:  # noqa: BLE001 - Prevent a single record error from stopping the entire file.
            ...
            logger.error("Failed to open EVTX file: %s -> %s", file_path, exc)
    ...
    def parse_directory(self, directory: Optional[Path] = None) -> Iterator[UserBehaviorEvent]:
        """In the `evtx_input_dir` directory, parse all .evtx files sequentially."""
    ...
    def stream_live(self, channel: Optional[str] = None) -> Iterator[UserBehaviorEvent]:
        ...
        try:
            ...
            raise ImportError(
                "pywin32 package not found. Install it: pip install pywin32"
            ) from exc
    ...
    # Persisting results (JSONL, partitioned by day)

    def persist(self, events: Iterator[UserBehaviorEvent]) -> Path:
        """
        Writes normalized events to `parsed_output_dir/YYYY-MM-DD.jsonl` format.
        The next feature extraction step will read these files.
        """
        ...
        logger.info("Persisted: %d events -> %s", count, out_path)
        return out_path
...
if __name__ == "__main__":
    ...
    print(f"Events persisted (filtered): {parser.stats.events_kept}")
    print(f"Events skipped (filtered): {parser.stats.events_skipped_filtered}")
    print(f"Events failed: {parser.stats.events_failed}")
